...
- `_heapify_up`, `_heapify_down` 구현이 필요하다.
...
- 키 기반 명령어는 실행 전 "만료 여부"를 먼저 확인할 수 있어야 한다. 만료된 키는 삭제 후 "없는 키"처럼 처리한다.
- 출력은 Redis 스타일을 따른다: `OK`, `(nil)`, `(integer) N`, `(error) ...`

#### `SET key value`

- 성공 시 `OK`
- 메모리 초과 시 LRU 제거를 수행한다. 세부 규칙은 메모리 관리 요구사항을 참조한다.
...
- 반환이 성공한 경우에만 LRU를 갱신한다. 만료로 삭제된 경우는 갱신하지 않는다.
...
- 현재 저장된 키 개수를 `(integer) N`으로 반환한다.

#### `KEYS`

- 전체 키 목록을 배열 형태로 출력한다. 정렬/순서는 요구하지 않는다.
- 출력 예시:
...
### 4.3 메모리 관리 및 LRU 자동 제거

#### `CONFIG SET maxmemory bytes`

- `bytes`는 0 이상의 정수이다.
- 0은 "무제한"으로 간주한다.
- 성공 시 `OK`, 정수 파싱 실패 시 에러 처리 표준을 따른다.

#### `INFO memory`

아래 3개 항목을 최소 포함해 출력한다. 표현 형태는 동일하면 된다.
...
- 만약 "단일 엔트리(키+값)" 자체가 `maxmemory`를 초과한다면 저장하지 않고 에러(OOM)를 출력한다.
...
- 키가 없으면 `(integer) 0`
- `seconds`가 0 이하라면 "즉시 만료"로 처리해도 된다. 존재하면 삭제 후 `(integer) 1`을 반환한다.
- 정상 설정 시 `(integer) 1`

#### `TTL key`

- 키가 없으면 `(integer) -2`
- 키는 존재하지만 만료 시간이 없으면 `(integer) -1`
- 만료 시간이 있으면 남은 초를 `(integer) N`으로 반환

#### TTL/LRU 엣지 케이스 최소 규칙

- 만료된 키는 `GET` 시 먼저 삭제 후 `(nil)`을 반환하며, LRU 갱신은 하지 않는다.
- `SET`이 기존 키를 덮어쓸 때 TTL은 초기화(삭제)한다.
- `EXPIRE`를 없는 키에 호출하면 `(integer) 0`이다.
- `DEL`은 데이터/TTL/LRU 모든 구조에서 엔트리를 함께 제거한다.
- 구현 방식은 "힙을 통해 가장 빠른 만료를 빠르게 찾을 수 있어야 한다"는 목표를 만족하면 된다. 예를 들어 lazy deletion 전략 등은 구현자가 선택할 수 있다.

### 4.5 에러 처리 표준 및 CLI 인터페이스
...
- 예시처럼 `"Alice"` 같은 따옴표 입력을 허용한다. 구현 난이도에 따라 단순 규칙으로 처리할 수 있다.
- 최소 요구: 공백이 없는 값 또는 큰따옴표로 감싼 값 중 하나의 방식을 지원한다.
...
- 네트워크 통신은 구현하지 않는다. 오직 CLI만 구현한다.
